...
\label{sec:orgc8fb9be}

\begin{frame}[label={sec:orge6f053a}]{Overview}
...
\label{sec:orgfa0b74c}

\begin{frame}[label={sec:org1c34799}]{Test Scores and district income}
\begin{columns}
...
\begin{frame}[label={sec:orgc4836f3}]{Why does a simple linear regression model not fit the data well?}
\begin{itemize}
...
\begin{frame}[label={sec:org51338ab}]{Estimate a quadratic regression model}
\begin{equation}
...
\begin{frame}[label={sec:org7ebc005}]{Estimate a quadratic regression model (cont'd)}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.6\textwidth,height=0.5\textwidth]{img/fig-8-3.png}
\caption{\label{fig:orgb0f7de2}
Scatterplot of test score vs district income and a quadratic regression line}
\end{figure}
\end{frame}

\begin{frame}[shrink,label={sec:org5f5ba03}]{A general formula for a nonlinear population regression function}
...
\begin{frame}[label={sec:orgd5b8a61}]{The effect on \(Y\) of a change in a regressor}
...
\begin{frame}[label={sec:org6588874}]{Application to test scores and income}
...
\begin{frame}[label={sec:org3b8667d}]{The effect of change in income on test scores}
...
\begin{frame}[label={sec:org10e3f0b}]{A general approach to modeling nonlinearities using multiple regression}
\begin{enumerate}
...
\label{sec:org4b3ffdb}

\begin{frame}[label={sec:org32cf2a2}]{Polynomials}
...
\begin{frame}[label={sec:org7d64da4}]{What is \(\Delta Y / \Delta X\) in a polynomial regression model?}
\begin{itemize}
...
\begin{frame}[label={sec:orge15ae6b}]{Which degree of polynomial should I use?}
\begin{itemize}
...
\begin{frame}[shrink,label={sec:orgd0957c9}]{Application to district income and test scores}
...
\begin{frame}[label={sec:org944dd0c}]{A natural logarithmic function \(y = \ln(x)\)}
\begin{itemize}
...
\begin{frame}[label={sec:orge2f172e}]{The percentage-change form using \(\ln(x)\)}
\begin{itemize}
...
\begin{frame}[label={sec:org33c56a9}]{The three logarithmic regression models}
...
\begin{frame}[label={sec:org0374bf0}]{Case I: linear-log model}
\begin{itemize}
...
\begin{frame}[label={sec:org2e2d579}]{Case II: log-linear model}
\begin{itemize}
...
\begin{frame}[shrink,label={sec:org3c3210e}]{Case III: log-log model}
\begin{itemize}
...
\begin{frame}[label={sec:org1b59634}]{The log-linear and log-log regression functions}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{img/fig-8-6.png}
\caption{\label{fig:org384ebd2}
The log-linear and log-log regression functions}
\end{figure}
\end{frame}

\begin{frame}[label={sec:org3080c84}]{Summary}
\begin{center}
...
\label{sec:org52d8c10}

\begin{frame}[label={sec:orgaba3aa7}]{Interactions between independent variables}
\begin{itemize}
...
\begin{frame}[label={sec:orga77fc77}]{The regression model with interaction between two binary variables}
...
\begin{frame}[label={sec:orgd20c4a3}]{The method of interpreting coefficients in regressions with interacted binary variables}
...
\begin{frame}[label={sec:org2b064f3}]{Compute the expected values of \(Y\) for each possible combinations of \(D_1\) and \(D_2\)}
\begin{description}
...
\begin{frame}[label={sec:org7a355ea}]{Compute the difference between a pair of cases}
\begin{description}
...
\begin{frame}[label={sec:org4681dda}]{Compute the difference between a pair of cases (cont'd)}
\begin{description}
...
\begin{frame}[label={sec:org6604dae}]{Hypothesis testing}
...
\begin{frame}[label={sec:org962b8bc}]{Interactions between a continuous and a binary variable}
...
\begin{frame}[label={sec:orgfd7be5d}]{Interactions between a continuous and a binary variable: graphic representation}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{img/fig-8-8.png}
\caption{\label{fig:orgd23b32a}
...
\label{sec:org858757f}
\begin{frame}[label={sec:org1888021}]{Nonlinear regression models and nonlinear least squares estimator}
...
\begin{frame}[label={sec:org2b1fe3b}]{Transform a nonlinear model to a linear one}
...
\begin{frame}[shrink,label={sec:org7750ed5}]{A nonlinear model: logistic function}
\begin{itemize}
...
\begin{frame}[label={sec:org8fe0c6b}]{A nonlinear model: negative exponential growth function}
\begin{itemize}
