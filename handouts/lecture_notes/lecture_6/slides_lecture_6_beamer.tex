...
\label{sec:orga4c3096}

\begin{frame}[label={sec:org07bd7cf}]{Definition of \alert{regress} in Merriam-Webster's dictionary}
...
\begin{frame}[label={sec:org40a5f9c}]{The meaning of regression in statistics?}
\begin{itemize}
...
\begin{frame}[label={sec:orged6413f}]{Research question:}
\begin{itemize}
...
\begin{frame}[label={sec:org2ad6e9f}]{Randomized controlled experiment}
\begin{itemize}
...
\begin{frame}[label={sec:orgfe403e7}]{Compute conditional means}
\begin{itemize}
...
\begin{frame}[label={sec:org2993eaa}]{The population regression function for test scores on class sizes}
\begin{itemize}
...
\begin{frame}[label={sec:org12784ce}]{The simple linear regression model for test scores on class sizes}
\begin{itemize}
...
\begin{frame}[label={sec:org5e86d6a}]{A distinction between the population regression function and the population regression model}
\begin{itemize}
...
\begin{frame}[label={sec:org0d80e86}]{An interpretation of the population regression model}
\begin{itemize}
...
\begin{frame}[label={sec:orgfe2128d}]{Interpret \(\beta_1\)}
\begin{itemize}
...
\begin{frame}[label={sec:orge53bbf3}]{Marginal effect}
\begin{itemize}
...
\begin{frame}[label={sec:org5510fe4}]{Holding other things constant}
\begin{itemize}
...
\begin{frame}[label={sec:org3bf8b37}]{Interpret \(\beta_0\)}
\begin{itemize}
...
\begin{frame}[label={sec:org08bd0ed}]{The general linear regression model}
\begin{itemize}
...
\begin{frame}[label={sec:org9e9d70c}]{The general linear regression model (cont'd)}
\begin{itemize}
...
\begin{frame}[label={sec:orgbf389a2}]{An graphical illustration of a linear regression model}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{figure/fig-4-1.png}
\caption{\label{fig:orgc782350}
...
\label{sec:org0c47a97}

\begin{frame}[label={sec:org08e6c63}]{The intuition for the OLS and minimization}
\begin{itemize}
...
\begin{frame}[label={sec:org4540cff}]{Ordinary}
\begin{itemize}
...
\begin{frame}[label={sec:org06e71bf}]{Least}
\begin{itemize}
...
\begin{frame}[label={sec:org87caef2}]{Squares}
\begin{itemize}
...
\begin{frame}[label={sec:orga553f2f}]{The OLS estimators for \(\beta_0\) and \(\beta_1\)}
\begin{itemize}
...
\begin{frame}[label={sec:orgbf45700}]{The first order conditions}
\begin{itemize}
...
\begin{frame}[label={sec:org13d56fe}]{Get the OLS estimator \(\hat{\beta}_0\)}
\begin{itemize}
...
\begin{frame}[label={sec:orgcebdb2f}]{Get the OLS estimator \(\hat{\beta}_1\)}
\begin{itemize}
...
\begin{frame}[label={sec:org7b22882}]{A trick of collecting terms}
\begin{align*}
...
\begin{frame}[label={sec:org28201b6}]{Concise expressions of \(\hat{\beta}_1\)}
\begin{itemize}
...
\begin{frame}[label={sec:orgfff282d}]{Summary of the OLS estimators}
\begin{itemize}
...
\begin{frame}[label={sec:org0e0b2ee}]{The predicted values, residuals, and the sample regression line}
...
\begin{frame}[label={sec:org287be9f}]{A comparison between the population regression model and the sample counterparts}
\begin{center}
\begin{tabular}{lll}
 & Population & Sample\\
\hline
...
\begin{frame}[label={sec:orgc5eadc8}]{The OLS estimates of the relationship between test scores and the student-teacher ratio}
$$TestScore = \beta_0 + \beta_1 ClassSize + OtherFactors$$
...
\begin{frame}[label={sec:org9376ae5}]{Basic summary statistics}
\begin{itemize}
...
\begin{frame}[label={sec:org121aba5}]{Scatterplot}
\begin{center}
...
\begin{frame}[label={sec:org01ddcd8}]{Regression analysis}
...
\begin{frame}[label={sec:orgf304d1c}]{Interpretation of the estimated coefficients}
\begin{itemize}
...
\begin{frame}[label={sec:orgdb238bb}]{Proof of \(\frac{1}{n} \sum_{i=1}^n \hat{Y}_i = \overline{Y}\)}
...
\begin{frame}[label={sec:org7fcd34e}]{Proof of \(\sum_{i=1}^n \hat{u}_i X_i = 0\)}
\begin{align*}
...
\begin{frame}[label={sec:org37a8a8a}]{Proof of \(TSS = ESS + SSR\)}
\begin{equation*}
\begin{split}
...
\label{sec:orgfce4461}

\begin{frame}[label={sec:org33b7f53}]{Goodness of Fit: R\(^{\text{2}}\)}
\begin{equation}
\label{eq:rsquared}
...
\begin{frame}[label={sec:orgd6e6a06}]{The use of \(R^2\)}
\begin{itemize}
...
\begin{frame}[label={sec:org32fae65}]{The standard error of regression (SER) as a measure of fit}
\begin{equation}
\label{eq:ser}
...
\begin{frame}[label={sec:orgb3b8fcb}]{\(R^2\) and SER for the application of test scores v.s. class sizes}
\begin{itemize}
...
\label{sec:org74632dc}

\begin{frame}[label={sec:org975fba6}]{Assumption 1: The conditional mean of \(u_i\) given \(X_i\) is zero}
\begin{equation}
\label{eq:Eu}
E(u_i | X_i) = 0
\end{equation}
...
\begin{frame}[label={sec:orge741a48}]{An illustration of Assumption 1}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{figure/fig-4-4.png}
\caption{\label{fig:orgdbc40b2}
An illustration of \(E(u|X=x)=0\)}
\end{figure}
\end{frame}

\begin{frame}[label={sec:orgd895913}]{Correlation and conditional mean}
...
\begin{frame}[label={sec:org121d985}]{Assumption 2: \((X_i, Y_i)\) for \(i = 1, \ldots, n\) are i.i.d.}
\begin{itemize}
...
\begin{frame}[label={sec:orgbab27dd}]{Assumption 3: large outliers are unlikely}
...
\begin{frame}[label={sec:org23fa0db}]{The influential observations and the leverage effects}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{figure/fig-4-5.png}
\caption{\label{fig:org29ea1d3}
...
\label{sec:orgcdc83bc}

\begin{frame}[label={sec:orgd2a575e}]{Unbiasedness}
...
\begin{frame}[label={sec:org71f42c4}]{Show that \(\hat{\beta}_1\) is unbiased}
\[\hat{\beta}_1  = \frac{\sum_{i=1}^n (X_i - \overline{X})(Y_i - \overline{Y})}{\sum_{i=1}^n (X_i - \overline{X})^2}\]
...
\begin{frame}[shrink,label={sec:org9ce9b39}]{Show that \(\hat{\beta}_1\) is unbiased (cont'd)}
\begin{itemize}
...
\begin{frame}[label={sec:org7064d0e}]{Show that \(\hat{\beta}_1\) is unbiased (cont'd)}
\begin{equation}
\label{eq:betahat-1b}
...
\begin{frame}[label={sec:org219c473}]{Show that \(\hat{\beta}_1\) is unbiased (cont'd)}
\begin{itemize}
...
\begin{frame}[label={sec:orgdc5d404}]{The consistency of \(\hat{\beta}_0\) and \(\hat{\beta}_1\)}
\begin{itemize}
...
\begin{frame}[label={sec:org06de251}]{The asymptotic normal distribution of \(\hat{\beta}_1\)}
\begin{itemize}
...
\begin{frame}[label={sec:orgf475e62}]{The asymptotic normal distribution of \(\hat{\beta}_0\)}
\begin{itemize}
