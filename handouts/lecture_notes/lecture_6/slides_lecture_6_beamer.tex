...
\label{sec:orgebf8c27}

\begin{frame}[label={sec:orgd40873f}]{Definition of \alert{regress} in Merriam-Webster's dictionary}
...
\begin{frame}[label={sec:org0eed673}]{The meaning of regression in statistics?}
\begin{itemize}
...
\begin{frame}[label={sec:org9fb054c}]{Research question:}
\begin{itemize}
...
\begin{frame}[label={sec:org8f3134c}]{Randomized controlled experiment}
\begin{itemize}
...
\begin{frame}[label={sec:org633fcfa}]{Compute conditional means}
\begin{itemize}
...
\begin{frame}[label={sec:org6cb7001}]{The population regression function for test scores on class sizes}
\begin{itemize}
...
\begin{frame}[label={sec:org4863838}]{The simple linear regression model for test scores on class sizes}
\begin{itemize}
...
\begin{frame}[label={sec:orgdb094ab}]{A distinction between the population regression function and the population regression model}
\begin{itemize}
...
\begin{frame}[label={sec:orgd084a7d}]{An interpretation of the population regression model}
\begin{itemize}
...
\begin{frame}[label={sec:orgd9ce304}]{Interpret \(\beta_1\)}
\begin{itemize}
...
\begin{frame}[label={sec:orgfed7540}]{Marginal effect}
\begin{itemize}
...
\begin{frame}[label={sec:org42df9c6}]{Holding other things constant}
\begin{itemize}
...
\begin{frame}[label={sec:org4bdbdf8}]{Interpret \(\beta_0\)}
\begin{itemize}
...
\begin{frame}[label={sec:org64215d8}]{The general linear regression model}
\begin{itemize}
...
\begin{frame}[label={sec:orgf286cff}]{The general linear regression model (cont'd)}
\begin{itemize}
...
\begin{frame}[label={sec:orgafbc7b5}]{An graphical illustration of a linear regression model}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{figure/fig-4-1.png}
\caption{\label{fig:org1609457}
...
\label{sec:org2a86303}

\begin{frame}[label={sec:org13a2d11}]{The intuition for the OLS and minimization}
\begin{itemize}
...
\begin{frame}[label={sec:org46d75ac}]{Ordinary}
\begin{itemize}
...
\begin{frame}[label={sec:org89898ab}]{Least}
\begin{itemize}
...
\begin{frame}[label={sec:orge5ba6f7}]{Squares}
\begin{itemize}
...
\begin{frame}[label={sec:org442dccf}]{The OLS estimators for \(\beta_0\) and \(\beta_1\)}
\begin{itemize}
...
\begin{frame}[label={sec:orga6aed5a}]{The first order conditions}
\begin{itemize}
...
\begin{frame}[label={sec:org177747c}]{Get the OLS estimator \(\hat{\beta}_0\)}
\begin{itemize}
...
\begin{frame}[label={sec:org53e99b3}]{Get the OLS estimator \(\hat{\beta}_1\)}
\begin{itemize}
...
\begin{frame}[label={sec:org2f043e6}]{A trick of collecting terms}
\begin{align*}
...
\begin{frame}[label={sec:orga4e4b12}]{Concise expressions of \(\hat{\beta}_1\)}
\begin{itemize}
...
\begin{frame}[label={sec:org0bc7440}]{Summary of the OLS estimators}
\begin{itemize}
...
\begin{frame}[label={sec:org3734a9f}]{The predicted values, residuals, and the sample regression line}
...
\begin{frame}[label={sec:org0dc2cc0}]{A comparison between the population regression model and the sample counterparts}
\begin{center}
\begin{tabular}{lll}
 & Population & Sample\\
\hline
...
\begin{frame}[label={sec:orgce4d37e}]{The OLS estimates of the relationship between test scores and the student-teacher ratio}
$$TestScore = \beta_0 + \beta_1 ClassSize + OtherFactors$$
...
\begin{frame}[label={sec:org49d91ef}]{Basic summary statistics}
\begin{itemize}
...
\begin{frame}[label={sec:org96f5838}]{Scatterplot}
\begin{center}
...
\begin{frame}[label={sec:orgcd72291}]{Regression analysis}
...
\begin{frame}[label={sec:orgab9cbf4}]{Interpretation of the estimated coefficients}
\begin{itemize}
...
\label{sec:orgca77d66}

\begin{frame}[label={sec:orgdaf7288}]{The algebraic properties of the ols estimator}
\begin{itemize}
...
\begin{frame}[label={sec:orge026b51}]{TSS, ESS, and SSR}
\begin{itemize}
...
\begin{frame}[label={sec:org35e3df6}]{Some algebraic properties among \(\hat{u}_i\), \(\hat{Y}_i\), and \(Y_i\)}
\begin{gather}
...
\begin{frame}[label={sec:orge2c1f93}]{Proof of \(\sum_{i=1}^n \hat{u}_i = 0\)}
...
\begin{frame}[label={sec:org9fd21f7}]{Proof of \(\frac{1}{n} \sum_{i=1}^n \hat{Y}_i = \overline{Y}\)}
...
\begin{frame}[label={sec:org58067f9}]{Proof of \(\sum_{i=1}^n \hat{u}_i X_i = 0\)}
\begin{align*}
...
\begin{frame}[label={sec:orgcbadb3d}]{Proof of \(TSS = ESS + SSR\)}
\begin{equation*}
\begin{split}
...
\label{sec:orge97a6ce}

\begin{frame}[label={sec:orgd4a06e9}]{Goodness of Fit: R\(^{\text{2}}\)}
\begin{equation}
\label{eq:rsquared}
...
\begin{frame}[label={sec:org329d412}]{The use of \(R^2\)}
\begin{itemize}
...
\begin{frame}[label={sec:orgbe7e65f}]{The standard error of regression (SER) as a measure of fit}
\begin{equation}
\label{eq:ser}
...
\begin{frame}[label={sec:orgdde5ac8}]{\(R^2\) and SER for the application of test scores v.s. class sizes}
\begin{itemize}
...
\label{sec:org0cea7b2}

\begin{frame}[label={sec:org72ea54a}]{Assumption 1: The conditional mean of \(u_i\) given \(X_i\) is zero}
\begin{equation}
\label{eq:Eu}
E(u_i | X_i) = 0
\end{equation}
...
\begin{frame}[label={sec:org65f7086}]{An illustration of Assumption 1}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{figure/fig-4-4.png}
\caption{\label{fig:org5bd804b}
An illustration of \(E(u|X=x)=0\)}
\end{figure}
\end{frame}

\begin{frame}[label={sec:orgad55156}]{Correlation and conditional mean}
...
\begin{frame}[label={sec:orgcfb6fdb}]{Assumption 2: \((X_i, Y_i)\) for \(i = 1, \ldots, n\) are i.i.d.}
\begin{itemize}
...
\begin{frame}[label={sec:orgfc00b7d}]{Assumption 3: large outliers are unlikely}
...
\begin{frame}[label={sec:org7529755}]{The influential observations and the leverage effects}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{figure/fig-4-5.png}
\caption{\label{fig:org7ce1bb6}
...
\label{sec:orgf457679}

\begin{frame}[label={sec:orgaf52977}]{Unbiasedness}
...
\begin{frame}[label={sec:org1b6b0fa}]{Show that \(\hat{\beta}_1\) is unbiased}
\[\hat{\beta}_1  = \frac{\sum_{i=1}^n (X_i - \overline{X})(Y_i - \overline{Y})}{\sum_{i=1}^n (X_i - \overline{X})^2}\]
...
\begin{frame}[shrink,label={sec:org7495931}]{Show that \(\hat{\beta}_1\) is unbiased (cont'd)}
\begin{itemize}
...
\begin{frame}[label={sec:org72281a2}]{Show that \(\hat{\beta}_1\) is unbiased (cont'd)}
\begin{equation}
\label{eq:betahat-1b}
...
\begin{frame}[label={sec:org27328eb}]{Show that \(\hat{\beta}_1\) is unbiased (cont'd)}
\begin{itemize}
...
\begin{frame}[label={sec:org12ecdc4}]{The consistency of \(\hat{\beta}_0\) and \(\hat{\beta}_1\)}
\begin{itemize}
...
\begin{frame}[label={sec:orgac2c740}]{The asymptotic normal distribution of \(\hat{\beta}_1\)}
\begin{itemize}
...
\begin{frame}[label={sec:orgadc6a66}]{The asymptotic normal distribution of \(\hat{\beta}_0\)}
\begin{itemize}
