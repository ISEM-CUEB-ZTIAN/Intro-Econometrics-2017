...
\label{sec:orgc6ccd45}

\begin{frame}[label={sec:org7e1ed6d}]{Definition of \alert{regress} in Merriam-Webster's dictionary}
...
\begin{frame}[label={sec:org63e9d84}]{The meaning of regression in statistics?}
\begin{itemize}
...
\begin{frame}[label={sec:org10d91ef}]{Research question:}
\begin{itemize}
...
\begin{frame}[label={sec:orgcc7e67a}]{Randomized controlled experiment}
\begin{itemize}
...
\begin{frame}[label={sec:orgb8e5372}]{Compute conditional means}
\begin{itemize}
...
\begin{frame}[label={sec:org8e8d6e3}]{The population regression function for test scores on class sizes}
\begin{itemize}
...
\begin{frame}[label={sec:orgdbd28e6}]{The simple linear regression model for test scores on class sizes}
\begin{itemize}
...
\begin{frame}[label={sec:org7a954f3}]{A distinction between the population regression function and the population regression model}
\begin{itemize}
...
\begin{frame}[label={sec:org35179ae}]{An interpretation of the population regression model}
\begin{itemize}
...
\begin{frame}[label={sec:org7312012}]{Interpret \(\beta_1\)}
\begin{itemize}
...
\begin{frame}[label={sec:org1a6aa9c}]{Marginal effect}
\begin{itemize}
...
\begin{frame}[label={sec:org5b465f4}]{Holding other things constant}
\begin{itemize}
...
\begin{frame}[label={sec:org7b0d139}]{Interpret \(\beta_0\)}
\begin{itemize}
...
\begin{frame}[label={sec:orga71101b}]{The general linear regression model}
\begin{itemize}
...
\begin{frame}[label={sec:org44a70a6}]{The general linear regression model (cont'd)}
\begin{itemize}
...
\begin{frame}[label={sec:orge00c558}]{An graphical illustration of a linear regression model}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{figure/fig-4-1.png}
\caption{\label{fig:org9da6d31}
...
\label{sec:org9a1a5e3}

\begin{frame}[label={sec:org439bbc6}]{The intuition for the OLS and minimization}
\begin{itemize}
...
\begin{frame}[label={sec:org82b29d7}]{Ordinary}
\begin{itemize}
...
\begin{frame}[label={sec:org53d019e}]{Least}
\begin{itemize}
...
\begin{frame}[label={sec:orgb6c196f}]{Squares}
\begin{itemize}
...
\begin{frame}[label={sec:org1e71358}]{The OLS estimators for \(\beta_0\) and \(\beta_1\)}
\begin{itemize}
...
\begin{frame}[label={sec:orga1b21c9}]{The first order conditions}
\begin{itemize}
...
\begin{frame}[label={sec:orgbfba5fc}]{Get the OLS estimator \(\hat{\beta}_0\)}
\begin{itemize}
...
\begin{frame}[label={sec:orgdead564}]{Get the OLS estimator \(\hat{\beta}_1\)}
\begin{itemize}
...
\begin{frame}[label={sec:org5ae007e}]{A trick of collecting terms}
\begin{align*}
...
\begin{frame}[label={sec:org7246723}]{Concise expressions of \(\hat{\beta}_1\)}
\begin{itemize}
...
\begin{frame}[label={sec:org9becb49}]{Summary of the OLS estimators}
\begin{itemize}
...
\begin{frame}[label={sec:org044f250}]{The predicted values, residuals, and the sample regression line}
...
\begin{frame}[label={sec:orge928208}]{A comparison between the population regression model and the sample counterparts}
\begin{center}
\begin{tabular}{lll}
 & Population & Sample\\
\hline
...
\begin{frame}[label={sec:org28bc1a3}]{The OLS estimates of the relationship between test scores and the student-teacher ratio}
$$TestScore = \beta_0 + \beta_1 ClassSize + OtherFactors$$
...
\begin{frame}[label={sec:org3f89c7f}]{Basic summary statistics}
\begin{itemize}
...
\begin{frame}[label={sec:org3767e25}]{Scatterplot}
\begin{center}
...
\begin{frame}[label={sec:org411521b}]{Regression analysis}
...
\begin{frame}[label={sec:org6be993c}]{Interpretation of the estimated coefficients}
\begin{itemize}
...
\label{sec:org19d523f}

\begin{frame}[label={sec:org7f077f8}]{The algebraic properties of the ols estimator}
\begin{itemize}
...
\begin{frame}[label={sec:org635b856}]{TSS, ESS, and SSR}
\begin{itemize}
...
\begin{frame}[label={sec:org1a69c24}]{Some algebraic properties among \(\hat{u}_i\), \(\hat{Y}_i\), and \(Y_i\)}
\begin{gather}
...
\begin{frame}[label={sec:org225c69d}]{Proof of \(\sum_{i=1}^n \hat{u}_i = 0\)}
...
\begin{frame}[label={sec:orgad4116a}]{Proof of \(\frac{1}{n} \sum_{i=1}^n \hat{Y}_i = \overline{Y}\)}
...
\begin{frame}[label={sec:orgdfe4cf2}]{Proof of \(\sum_{i=1}^n \hat{u}_i X_i = 0\)}
\begin{align*}
...
\begin{frame}[label={sec:orgabf18c7}]{Proof of \(TSS = ESS + SSR\)}
\begin{equation*}
\begin{split}
...
\label{sec:org6997efc}

\begin{frame}[label={sec:org4a8c303}]{Goodness of Fit: R\(^{\text{2}}\)}
\begin{equation}
\label{eq:rsquared}
...
\begin{frame}[label={sec:orga9d02c6}]{The use of \(R^2\)}
\begin{itemize}
...
\begin{frame}[label={sec:orgb4f6b90}]{The standard error of regression (SER) as a measure of fit}
\begin{equation}
\label{eq:ser}
...
\begin{frame}[label={sec:orge517cad}]{\(R^2\) and SER for the application of test scores v.s. class sizes}
\begin{itemize}
...
\label{sec:org3cc35b0}

\begin{frame}[label={sec:org8a1264c}]{Assumption 1: The conditional mean of \(u_i\) given \(X_i\) is zero}
\begin{equation}
\label{eq:Eu}
E(u_i | X_i) = 0
\end{equation}
...
\begin{frame}[label={sec:orge4436c9}]{An illustration of Assumption 1}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{figure/fig-4-4.png}
\caption{\label{fig:orgf27d164}
An illustration of \(E(u|X=x)=0\)}
\end{figure}
\end{frame}

\begin{frame}[label={sec:orga4ecbd3}]{Correlation and conditional mean}
...
\label{sec:orgcde7251}

\begin{frame}[label={sec:org9300ac9}]{Unbiasedness}
...
\begin{frame}[label={sec:orga5540d4}]{Show that \(\hat{\beta}_1\) is unbiased}
\[\hat{\beta}_1  = \frac{\sum_{i=1}^n (X_i - \overline{X})(Y_i - \overline{Y})}{\sum_{i=1}^n (X_i - \overline{X})^2}\]
...
\begin{frame}[shrink,label={sec:org0447e0d}]{Show that \(\hat{\beta}_1\) is unbiased (cont'd)}
\begin{itemize}
...
\begin{frame}[label={sec:org5904213}]{Show that \(\hat{\beta}_1\) is unbiased (cont'd)}
\begin{equation}
\label{eq:betahat-1b}
...
\begin{frame}[label={sec:org3dae169}]{Show that \(\hat{\beta}_1\) is unbiased (cont'd)}
\begin{itemize}
...
\begin{frame}[label={sec:org3a1e01f}]{The consistency of \(\hat{\beta}_0\) and \(\hat{\beta}_1\)}
\begin{itemize}
...
\begin{frame}[label={sec:org2628e9e}]{The asymptotic normal distribution of \(\hat{\beta}_1\)}
\begin{itemize}
...
\begin{frame}[label={sec:org8ceb39a}]{The asymptotic normal distribution of \(\hat{\beta}_0\)}
\begin{itemize}
