...
\label{sec:org5871418}
\setcounter{tocdepth}{1}
\tableofcontents[currentsection]

\begin{frame}[label={sec:orgba83b4c}]{The problem of a simple linear regression}
...
\begin{frame}[label={sec:orgfd22377}]{Percentage of English learners as an example}
...
\begin{frame}[label={sec:org0357308}]{Solutions to the problem of ignoring important factors}
...
\begin{frame}[label={sec:org401d7e4}]{A multiple regression model}
...
\begin{frame}[label={sec:org4209af2}]{The components in a multiple regression model}
\begin{itemize}
...
\begin{frame}[label={sec:org5a774d9}]{The interpretation of \(\beta_i\): Holding other things constant}
\begin{equation}
...
\begin{frame}[label={sec:org18cd185}]{The partial effect}
...
\begin{frame}[label={sec:org0f20940}]{Look at the data in terms of vectors and matrix}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.4\textwidth,height=0.5\textheight]{img/data_snapshot.png}
\caption{\label{fig:org8673189}
...
\begin{frame}[shrink,label={sec:orgcde926b}]{Define variables in matrix notation}
...
\begin{frame}[label={sec:org42aa23d}]{Two other ways to write the regression model}
...
\begin{frame}[label={sec:orgf622d07}]{Two other ways to write the regression model (cont'd)}
...
\label{sec:org729e33b}
\setcounter{tocdepth}{1}
\tableofcontents[currentsection]

\begin{frame}[label={sec:org9a91cc5}]{The minimization problem and the OLS estimator}
\begin{itemize}
...
\begin{frame}[label={sec:org6b5d5da}]{The minimization problem and the OLS estimator (cont'd)}
\begin{itemize}
...
\begin{frame}[label={sec:orgfe5a9a4}]{The OLS estimator of \(\boldsymbol{\beta}\) as a solution to the minimization problem}
\begin{itemize}
...
\begin{frame}[label={sec:orge55fe43}]{The OLS estimator in matrix notation}
...
\begin{frame}[label={sec:org7a64328}]{The proof}
...
\begin{frame}[label={sec:org3675aa1}]{The OLS estimator of \(\hat{\beta}_1\) in a simple regression model}
...
\begin{frame}[plain,label={sec:org9d65049}]{The OLS estimator of \(\hat{\beta}_1\) in a simple regression model (cont'd)}
...
\begin{frame}[plain,label={sec:orgb6de805}]{The OLS estimator of \(\hat{\beta}_1\) in a simple regression model (cont'd)}
\begin{description}
...
\begin{frame}[plain,label={sec:org063559b}]{The OLS estimator of \(\hat{\beta}_1\) in a simple regression model (cont'd)}
...
\begin{frame}[plain,label={sec:org2820f09}]{The OLS estimator of \(\hat{\beta}_1\) in a simple regression model (cont'd)}
...
\begin{frame}[shrink,plain,label={sec:org7412b50}]{Application to Test Scores and the Student-Teacher Ratio}
...
\label{sec:org9ef4c68}
\setcounter{tocdepth}{1}
\tableofcontents[currentsection]

\begin{frame}[label={sec:org96768c1}]{The standard errors of the regression (SER)}
\begin{itemize}
...
\begin{frame}[label={sec:orgfc59514}]{\(R^2\)}
...
\begin{frame}[label={sec:org78a372e}]{Limitations of R\(^{\text{2}}\)}
\begin{itemize}
...
\begin{frame}[label={sec:orgf63104e}]{Limitation of R\(^{\text{2}}\) (cont'd)}
\begin{itemize}
...
\begin{frame}[label={sec:org9111f7a}]{Limitation of R\(^{\text{2}}\) (cont'd)}
\begin{itemize}
...
\begin{frame}[label={sec:orgc4d2823}]{The adjusted R\(^{\text{2}}\)}
\begin{itemize}
...
\begin{frame}[label={sec:orgf5032b7}]{Properties of \(\bar{R}^2\)}
\begin{itemize}
...
\begin{frame}[label={sec:orge085f30}]{The usefulness of the R\(^{\text{2}}\) and \(\bar{R}^2\)}
\begin{itemize}
...
\label{sec:org7d2a190}
\setcounter{tocdepth}{1}
\tableofcontents[currentsection]

\begin{frame}[label={sec:org6a4a240}]{The grouped regressors}
...
\begin{frame}[label={sec:orgf97f0ef}]{Two estimation strategies}
...
\begin{frame}[label={sec:org89481a3}]{The second strategy: the step OLS estimation}
\begin{enumerate}
...
\begin{frame}[label={sec:org78d01c9}]{The Frisch-Waugh-Lovell Theorem}
...
\begin{frame}[label={sec:org731ea07}]{An understanding of the FWL theorem}
...
\begin{frame}[label={sec:org4bafcf1}]{An example of the FWL theorem}
