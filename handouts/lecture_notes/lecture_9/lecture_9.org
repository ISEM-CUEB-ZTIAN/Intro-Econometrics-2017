...
- $\hat{\boldsymbol{\beta}}$ is the estimated coefficients by OLS and
  ...
  - The F-statistic computed as in Equation eqref:eq:ftest-gen is
    a *heteroskedasticity-robust* F-statistic.
...
n$, then we can compute *the homoskedasticity-only F-statistic* that
...
3. In other words, given the same sample, $R^2$ in the unrestricted
   ...
   coefficient.
4. That means $RSSR \geq USSR$ and $R^2_{unrestrict} \geq R^2_{restrict}$
   are always true.
5. However, suppose that the null hypothesis is true. That is, the
   true model is really the restricted one.
6. Then, the explanatory power of the additional regressors in the
...
- This set has coverage rate 95% because the test on which it is based
  has the size of 5%.
- Therefore the confidence set which is constructed as the non-rejected
...
- One group consists of regressors whose causal effects on $Y$ are our
  research interest so that we want unbiased estimates of these
  coefficients.
- Another group consists of regressors whose causal effects on $Y$ are
...
the regressors in the first groups are unbiased, maintaining the
causal implication that we want. This new assumption is *conditional
mean independence*.


** COMMENT A conundrum regarding to the OLS estimation and omitted variable bias
...
variable, which is *conditional mean independence*.
...
# In summary, if $W$ is such a control variable that conditional mean
# independence is satisfied, then:
# - The OLS estimator of the coefficient of interest, $\hat{\beta}_1$, is
#   unbiased.
...
1. a  base set of regressors should be chosen using a
   ...
   2) thinking of the omitted causal effects that could result in omitted
      variable bias.
   3) including those omitted causal effects if you can find relevant
      ...
      plausibly holds.
2. Specify a range of plausible *alternative model specifications*,
...
It is easy to fall into the trap of maximizing the $R^2$ and
$\bar{R}^2$, but this loses sight of our real objective, an unbiased
estimator of the class size effect.
- A high $R^2$ or $\bar{R}^2$ means that the regressors explain the
  variation in Y.
- A high $R^2$ or $\bar{R}^2$ does not mean that you have eliminated
  omitted variable bias.
- A high $R^2$ or $\bar{R}^2$ does not mean that you have an unbiased
  estimator of a causal effect ($\beta_1$).
...
#+ATTR_LATEX: :width 1.0\textwidth :float t :placement [!ht]
